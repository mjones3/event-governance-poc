---
id: PQCCompletedConsequence
name: PQCCompletedConsequence
version: '1.0'
summary: Event published by qualitycontrol service when pqccompletedconsequence occurs
owners:
  - qualitycontrol-service
producers:
  - qualitycontrol-service
consumers:
  []
badges:
  - content: "Schema Valid ✓"
    backgroundColor: "#22c55e"
    textColor: white
  - content: "Domain: Manufacturing"
    backgroundColor: "#3b82f6"
    textColor: white
  - content: "v1.0"
    backgroundColor: "#6366f1"
    textColor: white
---

# PQCCompletedConsequenceEvent

**Schema ID**: 125
**Schema Version**: 1
**Subject**: PQCCompletedConsequenceEvent
**Domain**: Manufacturing
**Namespace**: com.arcone.biopro.manufacturing.qualitycontrol.domain.event

## Business Context

Event published when pqccompletedconsequence occurs

## Event Schema Fields

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `eventId` | `uuid` | Yes | Unique identifier for this event |
| `occurredOn` | `timestamp-millis` | Yes | Timestamp when event occurred |
| `occurredOnTimeZone` | `string` | No | Timezone for occurredOn |
| `eventType` | `string` | No | Type of event |
| `eventVersion` | `string` | No | Version of the event schema |
| `Type` | `string` | Yes | Type field |
| `of` | `string` | Yes | of field |
| `consequence` | `string` | Yes | consequence field |
| `apply` | `string` | Yes | apply field |
| `the` | `string` | Yes | the field |
| `values` | `string` | Yes | values field |


## Sample Event

```json
{
  "eventId": "e7050925-4b93-4abb-ae9e-ded9c4992a87",
  "occurredOn": 1763355987555,
  "occurredOnTimeZone": 1763355987555,
  "eventType": "STANDARD",
  "eventVersion": "1.0",
  "Type": "STANDARD",
  "of": "of_value",
  "consequence": "consequence_value",
  "apply": "apply_value",
  "the": "the_value",
  "values": "values_value"
}
```

## Schema Evolution

This event uses Avro schema validation through Confluent Schema Registry.

- **Schema ID**: 125
- **Version**: 1
- **Compatibility**: BACKWARD (default)

## Message Format

Messages are serialized using **Avro** with the schema ID embedded in the message header (magic byte + schema ID).

## Integration Notes

### Consuming This Event

When consuming this event, use the Confluent Avro deserializer which will:
1. Read the schema ID from the message
2. Fetch the schema from the registry (with caching)
3. Deserialize the message using the schema

### Publishing This Event

When publishing this event:
1. Use the Confluent Avro serializer
2. The serializer will register the schema if needed
3. The schema ID will be embedded in each message

## Download Schema

- [Download Avro Schema](./PQCCompletedConsequenceEvent.avsc) - Complete Avro schema definition

<NodeGraph />
