...
name: ShipmentPackingStarted
version: '1.0'
summary: Event published by shipping service when shipmentpackingstarted occurs
owners:
  - shipping-service
producers:
  - shipping-service
...
# ShipmentPackingStarted Event

**Schema ID**: 999
**Schema Version**: 1.0
**Subject**: ShipmentPackingStartedEvent
**Domain**: Distribution
...
Event published when shipmentpackingstarted occurs in the shipping service.
...
import com.arcone.biopro.shipping.domain.event.ShipmentPackingStartedEvent;
...
public class ShipmentPackingStartedEventListener extends AbstractListener<ShipmentPackingStartedEvent> {

    private final YourBusinessService businessService;

    @KafkaListener(
        topics = "${kafka.topics.shipmentpackingstarted}",
        groupId = "${kafka.consumer.group-id}",
        containerFactory = "kafkaListenerContainerFactory",
        errorHandler = "kafkaListenerErrorHandler"
    )
    public void listen(
            @Payload ShipmentPackingStartedEvent event,
            @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,
            @Header(KafkaHeaders.RECEIVED_PARTITION) int partition,
            @Header(KafkaHeaders.OFFSET) long offset
    ) {
        log.info("Received ShipmentPackingStartedEvent: eventId={}, topic={}, partition={}, offset={}",
                event.getEventId(), topic, partition, offset);

        processMessage(event)
            .doOnSuccess(result ->
                log.info("Successfully processed ShipmentPackingStartedEvent: {}", event.getEventId()))
            .doOnError(error ->
                log.error("Failed to process ShipmentPackingStartedEvent: {}", event.getEventId(), error))
            .subscribe();
    }

    @Override
    protected Mono<ShipmentPackingStartedEvent> processMessage(ShipmentPackingStartedEvent event) {
        return Mono.defer(() -> {
            try {
                validateEvent(event);
    ...
    private void validateEvent(ShipmentPackingStartedEvent event) {
...
{
  "type": "record",
  "name": "ShipmentPackingStartedEvent",
  "namespace": "com.arcone.biopro.distribution.shipping.domain.event",
  "doc": "Event published when shipmentpackingstarted occurs",
  "fields": [
    {
      "name": "eventId",
      "type": {
    ...
    },
    {
      ...
      "type": "string",
      "doc": "Type of event",
      "default": "SHIPMENTPACKINGSTARTED"
    },
    {
      "name": "eventVersion",
      "type": "string",
