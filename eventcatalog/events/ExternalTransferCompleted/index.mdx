...
version: '1.0'
summary: Event published by shipping service when externaltransfercompleted occurs
owners:
  - shipping-service
producers:
  - shipping-service
...
# ExternalTransferCompleted Event

**Schema ID**: 999
**Schema Version**: 1.0
**Subject**: ExternalTransferCompletedEvent
**Domain**: Distribution
...
Event published when externaltransfercompleted occurs in the shipping service.
...
import com.arcone.biopro.shipping.domain.event.ExternalTransferCompletedEvent;
...
public class ExternalTransferCompletedEventListener extends AbstractListener<ExternalTransferCompletedEvent> {

    private final YourBusinessService businessService;

    @KafkaListener(
        topics = "${kafka.topics.externaltransfercompleted}",
        groupId = "${kafka.consumer.group-id}",
        containerFactory = "kafkaListenerContainerFactory",
        errorHandler = "kafkaListenerErrorHandler"
    )
    public void listen(
            @Payload ExternalTransferCompletedEvent event,
            @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,
            @Header(KafkaHeaders.RECEIVED_PARTITION) int partition,
            @Header(KafkaHeaders.OFFSET) long offset
    ) {
        log.info("Received ExternalTransferCompletedEvent: eventId={}, topic={}, partition={}, offset={}",
                event.getEventId(), topic, partition, offset);

        processMessage(event)
            .doOnSuccess(result ->
                log.info("Successfully processed ExternalTransferCompletedEvent: {}", event.getEventId()))
            .doOnError(error ->
                log.error("Failed to process ExternalTransferCompletedEvent: {}", event.getEventId(), error))
            .subscribe();
    }

    @Override
    protected Mono<ExternalTransferCompletedEvent> processMessage(ExternalTransferCompletedEvent event) {
        return Mono.defer(() -> {
            try {
                validateEvent(event);
    ...
    private void validateEvent(ExternalTransferCompletedEvent event) {
...
{
  "type": "record",
  "name": "ExternalTransferCompletedEvent",
  "namespace": "com.arcone.biopro.distribution.shipping.domain.event",
  "doc": "Event published when externaltransfercompleted occurs",
  "fields": [
    {
      "name": "eventId",
      "type": {
    ...
    },
    {
      ...
      "type": "string",
      "doc": "Type of event",
      "default": "EXTERNALTRANSFERCOMPLETED"
    },
    {
      "name": "eventVersion",
      "type": "string",
