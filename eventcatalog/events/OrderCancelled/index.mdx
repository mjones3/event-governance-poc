...
name: OrderCancelled
version: '1.0'
summary: Event published by order service when ordercancelled occurs
owners:
  - order-service
producers:
  - order-service
...
Event published when ordercancelled occurs in the order service.
...
import com.arcone.biopro.order.domain.event.OrderCancelledEvent;
...
public class OrderCancelledEventListener extends AbstractListener<OrderCancelledEvent> {

    private final YourBusinessService businessService;

    @KafkaListener(
        topics = "${kafka.topics.ordercancelled}",
    ...
    public void listen(
        ...
        log.info("Received OrderCancelledEvent: eventId={}, topic={}, partition={}, offset={}",
                event.getEventId(), topic, partition, offset);

        processMessage(event)
            .doOnSuccess(result ->
                log.info("Successfully processed OrderCancelledEvent: {}", event.getEventId()))
            .doOnError(error ->
                log.error("Failed to process OrderCancelledEvent: {}", event.getEventId(), error))
            .subscribe();
    }

    @Override
    protected Mono<OrderCancelledEvent> processMessage(OrderCancelledEvent event) {
        return Mono.defer(() -> {
            try {
                validateEvent(event);
...
```json
{
  ...
  "doc": "Event published when ordercancelled occurs",
  "fields": [
    {
      "name": "eventId",
      "type": {
