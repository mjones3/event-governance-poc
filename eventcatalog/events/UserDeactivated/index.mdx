...
version: '1.0'
summary: Event published by role service when userdeactivated occurs
owners:
  - role-service
producers:
  - role-service
...
Event published when userdeactivated occurs in the role service.
...
import com.arcone.biopro.role.domain.event.UserDeactivatedEvent;
...
public class UserDeactivatedEventListener extends AbstractListener<UserDeactivatedEvent> {

    private final YourBusinessService businessService;

    @KafkaListener(
        topics = "${kafka.topics.userdeactivated}",
    ...
    public void listen(
        ...
        log.info("Received UserDeactivatedEvent: eventId={}, topic={}, partition={}, offset={}",
                event.getEventId(), topic, partition, offset);

        processMessage(event)
            .doOnSuccess(result ->
                log.info("Successfully processed UserDeactivatedEvent: {}", event.getEventId()))
            .doOnError(error ->
                log.error("Failed to process UserDeactivatedEvent: {}", event.getEventId(), error))
            .subscribe();
    }

    @Override
    protected Mono<UserDeactivatedEvent> processMessage(UserDeactivatedEvent event) {
        return Mono.defer(() -> {
            try {
                validateEvent(event);
    ...
    private void validateEvent(UserDeactivatedEvent event) {
...
```json
{
  ...
  "doc": "Event published when userdeactivated occurs",
  "fields": [
    {
      "name": "eventId",
      "type": {
